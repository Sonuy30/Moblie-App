...
export default function ProductDetailScreen() {
  ...
  const loadReviews = useCallback(async (pageToLoad = 1, shouldAppend = false, sortOption = 'newest') => {
    ...
    try {
      const res = await getReviews(product._id, pageToLoad, 3, sortOption);
      setReviews((prev) => (shouldAppend ? [...prev, ...res.reviews] : res.reviews));
  ...
  // Load reviews when screen is focused or parameters change
  useFocusEffect(
    useCallback(() => {
      if (product?._id) {
        void loadReviews(1, false, reviewsSort);
      }
  ...
  // ── Seller fallback helper ───────────────────────────────
  const mockSeller = {
    id: 'seller-aits-01',
    name: 'AITS Steel Traders',
    rating: 4.8,
    reviewCount: 1240,
    isVerified: true,
    joinDate: 'Jan 2022',
    description: 'Premier supplier of high-grade construction steel, TMT bars, structural angles, and industrial pipes. Delivering across India with certified quality standards.',
  };
  const activeSeller = product.seller || mockSeller;
  ...
  const computedAvgRating = ratingSummary?.average ?? product.avgRating ?? 0;
  const computedReviewCount = ratingSummary?.totalCount ?? product.reviewCount ?? 0;
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.section}>
            ...
            <SellerCard seller={activeSeller} />
          </View>
              ...
              <View style={{ marginTop: spacing.md }}>
                ...
                {reviewsPage < reviewsTotalPages && (
                  <TouchableOpacity
                    ...
                    onPress={() => void loadReviews(reviewsPage + 1, true, reviewsSort)}
                    ...
                    {reviewsLoading ? (
                      ...
                      <>
                        <Text style={styles.showMoreReviewsText}>
                          Load More Reviews ({reviewsTotalCount - reviews.length} remaining)
                        </Text>
